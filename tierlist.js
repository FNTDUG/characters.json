 (function() {
  ...
  var cfg = {
   ...
   // Counts distinct units, not entries. A hero listed once per build appears
   // several times across the tiers and must still count as one unit.
   function countUnits(m) {
     var ls = lists[m] || {};
     var seen = {}, n = 0;
     TIERS.forEach(function(t) {
       (ls[t.id] || []).forEach(function(raw) {
         var nm = parseUnit(raw).name;
         if (seen[nm]) return;
         seen[nm] = 1;
         n++;
       });
   ...
   var LINK_RE = /^linked$/i;
   var LINK_ICON = 'https://images.fntduserguide.com/chain-removebg-preview.png';
   ...
   var PATH_RE = /^P(\d+)(?:-(\d+))?$/i;
   // A cross-path build, same notation the meta teams use: tiers taken in path
   // 1-2-3. Needs at least one hyphen, so it cannot swallow a unit whose name
   // ends in a number (Endo 01, Endo 02).
   var BUILD_RE = /^\d+(?:-\d+)+$/;
   ...
   function parseUnit(str) {
     ...
     var tags  = [];
     var path  = '';
     var linkL = false, linkR = false;
     while (parts.length > 1 && LINK_RE.test(parts[0])) { linkL = true; parts.shift(); }
     while (parts.length > 1) {
       var last = parts[parts.length - 1];
       if (LINK_RE.test(last)) { linkR = true; parts.pop(); continue; }
       if (STATUS_TAGS.indexOf(last) !== -1) { tags.unshift(parts.pop()); continue; }
       if (!path && (PATH_RE.test(last) || BUILD_RE.test(last))) { path = parts.pop(); continue; }
       break;
     }
     return { name: parts.join(' '), tags: tags, path: path, linkL: linkL, linkR: linkR };
   }

   var PATH_IMG = 'https://images.fntduserguide.com/';

   // Two notations. P2 (or P2-3) ranks a single path and shows one icon. A build
   // like 0-5-2 is a cross-path setup and shows both, primary in front, exactly
   // as the meta team cards do.
   function tlPathInfo(name, pathStr) {
     ...
     function leg(idx, tier) {
       var path = u.paths[idx];
       ...
       if (!icon) return null;
       return { url: PATH_IMG + slug + '-paths/' + icon, pathNo: idx + 1, tier: tier };
     }
     ...
     if (m) {
       ...
       var pth = u.paths[idx];
       if (!pth || !Array.isArray(pth.tiers) || !pth.tiers.length) return null;
       var one = leg(idx, m[2] ? parseInt(m[2], 10) : pth.tiers.length);
       if (!one) return null;
       return {
         primary: one,
         secondary: null,
         label: 'Path ' + one.pathNo + (m[2] ? ' \u00b7 Tier ' + one.tier : '')
       };
     }

     if (!BUILD_RE.test(pathStr)) return null;
     ...
     return { primary: taken[0], secondary: taken[1] || null, label: tiers.join('-') };
   }
   ...
   var CSS = [
    ...
    '.tl-row[data-t=op] .tl-nm2{font-size:clamp(26px,5.2vw,32px)}',
     ...
     /* The tier art is already a self-contained circular emblem on its own dark
     ...
     '.tl-path{position:absolute;bottom:3px;right:3px;width:32px;height:32px;pointer-events:none;z-index:3}',
     ...
     '.tl-linked{display:flex;gap:6px;flex-wrap:nowrap;flex-shrink:0}',
     '.tl-link{position:absolute;top:50%;width:30px;height:30px;object-fit:contain;pointer-events:none;z-index:5;filter:drop-shadow(0 1px 3px rgba(0,0,0,1)) drop-shadow(0 0 1px rgba(0,0,0,1))}',
     '.tl-link-r{right:-3px;transform:translate(50%,-50%)}',
     '.tl-link-l{left:-3px;transform:translate(-50%,-50%)}',
     ...
     '#tlCam{display:inline-flex;align-items:center;justify-content:center;font-family:"Press Start 2P",monospace;padding:11px 12px;background:rgba(0,0,0,0.55);border:2px solid rgba(255,255,255,0.12);color:rgba(255,255,255,0.38);cursor:pointer;transition:all .15s}',
     '#tlCam:hover{border-color:#681f62;color:#681f62;box-shadow:0 0 12px rgba(104,31,98,0.5)}',
     '#tlCam[disabled]{opacity:.45;cursor:default}',
     '#tlCam svg{display:block;width:15px;height:15px}',
     '.tl-shot-overlay{position:fixed;inset:0;z-index:100000;background:rgba(0,0,0,0.86);display:flex;flex-direction:column;align-items:center;justify-content:center;gap:16px;padding:20px;box-sizing:border-box}',
     '.tl-shot-overlay img{max-width:92vw;max-height:66vh;border-radius:10px;box-shadow:0 10px 34px rgba(0,0,0,0.75)}',
     '.tl-shot-actions{display:flex;gap:12px;flex-wrap:wrap;justify-content:center}',
     '.tl-shot-btn{font-family:"Press Start 2P",monospace;font-size:11px;padding:12px 18px;border-radius:10px;cursor:pointer;border:2px solid;transition:background .12s,color .12s,border-color .12s}',
     '.tl-shot-save{background:#2a1f3a;border-color:#c862be;color:#c862be}',
     '.tl-shot-save:hover{background:#3a2a4a;color:#fff}',
     '.tl-shot-close{background:rgba(30,30,40,0.9);border-color:rgba(255,255,255,0.25);color:#ccc}',
     '.tl-shot-close:hover{color:#fff;border-color:rgba(255,255,255,0.5)}',
     '@media(max-width:600px){.tl-shot-btn{font-size:9px;padding:10px 14px}#tlCam{padding:6px 7px}#tlCam svg{width:11px;height:11px}}',
   ...
   function _tlShowShot(blob, filename) {
     if (!blob) return;
     ...
     if (!(canShareFiles && touch)) { _tlDlBlob(blob, filename); return; }

     var overlay = document.createElement('div'); overlay.className = 'tl-shot-overlay';
   ...
   function makeCamBtn() {
     ...
     btn.type = 'button';
     btn.id = 'tlCam';
     btn.setAttribute('aria-label', 'Screenshot tierlist');
     ...
     btn.addEventListener('click', function () {
       ...
       btn.disabled = true;
       var tlRoot = document.getElementById('tlRoot');
       if (!tlRoot) { btn.disabled = false; return; }
       ...
       withHtml2Canvas(function (ok) {
         ...
         imageTimeout: 30000,   // default is 15s, which slow connections overrun -> blank tiles
           ...
           onclone: function (doc) {
             ...
             // In the clone that offset would push the render half a viewport
             // sideways, so pin it to a plain fixed-width block instead.
             var cr = doc.getElementById('tlRoot');
             ...
             var cb = doc.getElementById('tlCam');    if (cb) cb.style.display = 'none';
             // Search highlighting is transient state, not part of the list.
             Array.prototype.forEach.call(doc.querySelectorAll('.tl-match'), function (el) {
               el.classList.remove('tl-match');
             });
           }
   ...
   var tipPath = document.createElement('div');
   tipPath.className = 'tl-tip-path';
   tipInner.appendChild(tipName);
   tipInner.appendChild(tipPath);
   ...
   function makeCard(raw_name, isSSZone) {
     ...
     var pInfo = tlPathInfo(name, parsed.path);
     var u   = unitMap[name] || {};
     ...
     if (rar !== 'shiny' && RARITY_FILL[rar]) inn.style.background = RARITY_FILL[rar];
     ...
     card.addEventListener('mouseenter', function() {
       ...
       if (pInfo) {
         tipPath.textContent = 'Hero Tree: ' + pInfo.label;
     ...
     // With a build shown, bottom-right belongs to it and the status icons take
     // the three corners that are left.
     var tagSlots = pInfo ? CORNERS.length - 1 : CORNERS.length;
     statuses.forEach(function(tag, i) {
       if (i >= tagSlots) return;
   ...
   function addLink(card, side) {
     var im = document.createElement('img');
     im.className = 'tl-link tl-link-' + side;
     im.alt = '';
     _tlImgSrc(im, LINK_ICON);
     card.appendChild(im);
   }
   ...
   function renderMode(m) {
     ...
     TIERS.forEach(function(t) {
       ...
       var prev = null, prevCard = null, group = null;
       names.forEach(function(name) {
         var p = parseUnit(name);
         var card = makeCard(name, t.id === 's' || t.id === 'op');
         if (prev && (prev.linkR || p.linkL)) {
           if (!group) {
             group = document.createElement('div');
             group.className = 'tl-linked';
             z.replaceChild(group, prevCard);
             group.appendChild(prevCard);
           }
           group.appendChild(card);
           if (prev.linkR) addLink(prevCard, 'r'); else addLink(card, 'l');
         } else {
           group = null;
           z.appendChild(card);
         }
         prev = p; prevCard = card;
       });
     });
   ...
   xhr.onload = function() {
     ...
     units.forEach(function(u) { unitMap[u.name] = {img: u.imgNormal || '', rarity: u.rarity || '', paths: u.paths || null}; });
     renderMode(mode);
   };
