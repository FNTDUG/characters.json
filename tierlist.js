(function() {
 ...
 var cfg = {
  "lists": {
    "all": {
      ...
      "s": [
        ...
        "Candy Cadet",
        "Dragon Endo 01 DOWN",
        "Purifier Mercenary Toy Chica UP",
        "Witch of the Sea Scrapbaby NEW",
        "Krampus Purple Guy",
        "Ennard",
  ...
  var CSS = [
    ...
    '.tl-cam-btn{margin-left:auto;display:inline-flex;align-items:center;justify-content:center;padding:8px 10px;background:rgba(0,0,0,0.55);border:2px solid rgba(246,155,85,0.75);border-radius:4px;color:#ffa45b;cursor:pointer;transition:background .15s,border-color .15s,color .15s}',
    '.tl-cam-btn:hover{background:rgba(246,155,85,0.14);color:#fff}',
    '.tl-cam-btn svg{display:block;width:18px;height:18px}',
    '@media(max-width:600px){.tl-cam-btn{padding:6px 8px}.tl-cam-btn svg{width:15px;height:15px}}',
  ...
  function captureRoot(rootEl, hideEls, filename) {
    ensureH2C(function () {
      hideEls = hideEls || [];
      var prev = hideEls.map(function (el) { return el ? el.style.display : null; });
      hideEls.forEach(function (el) { if (el) el.style.display = 'none'; });
      function restore() { hideEls.forEach(function (el, i) { if (el) el.style.display = prev[i]; }); }
      window.html2canvas(rootEl, { backgroundColor: '#03030a', useCORS: true, scale: 2, logging: false }).then(function (canvas) {
        restore();
        canvas.toBlob(function (blob) { saveImage(blob, filename); }, 'image/png');
      }).catch(function (e) { restore(); if (window.console) console.error('screenshot failed', e); });
    });
  }
  ...
  var tlCamBtn = document.createElement('button');
  tlCamBtn.type = 'button'; tlCamBtn.className = 'tl-cam-btn';
  tlCamBtn.title = 'Screenshot'; tlCamBtn.setAttribute('aria-label', 'Screenshot');
  tlCamBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>';
  tlCamBtn.addEventListener('click', function () { captureRoot(root, [tlCamBtn, searchEl], 'fntd-tierlist.png'); });
  hdr.appendChild(tlCamBtn);
